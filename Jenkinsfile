    ...
    stages {
        ...
        stage('multi-client') {
            steps {
                    ...
                    sh 'sudo -S docker build --file Dockerfile -t rupesh1050/multi-client:${BUILD_NUMBER} ./client'
        ...
        stage('multi-nginx') {
            steps {
                    withCredentials([usernamePassword(credentialsId: 'DOCKER_LOGIN', passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_NAME')]) {
                    sh 'sudo -S docker login -u ${DOCKER_NAME} -p ${DOCKER_PASSWORD}'
                    sh 'sudo -S docker build --file Dockerfile -t rupesh1050/multi-nginx:${BUILD_NUMBER} ./nginx'
        ...
        stage('multi-server') {
            steps {
                    withCredentials([usernamePassword(credentialsId: 'DOCKER_LOGIN', passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_NAME')]) {
                    sh 'sudo -S docker login -u ${DOCKER_NAME} -p ${DOCKER_PASSWORD}'
                    sh 'sudo -S docker build --file Dockerfile -t rupesh1050/multi-server:${BUILD_NUMBER} ./server'
        ...
        stage('multi-worker') {
            steps {
                    withCredentials([usernamePassword(credentialsId: 'DOCKER_LOGIN', passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_NAME')]) {
                    sh 'sudo -S docker login -u ${DOCKER_NAME} -p ${DOCKER_PASSWORD}'
                    sh 'sudo -S docker build . --file Dockerfile -t rupesh1050/multi-worker:${BUILD_NUMBER} ./worker'
